...
const getDateData = (pickDate) => {
  return moment(pickDate).format("YYYY-MM-DD")
}
class RecordsList extends Component {
  ...
  handleFilter = (values) => {
    ...
    getRecords({
      params: {
        from: getDateData(values.from),
        to: getDateData(values.to),
      }
    })
  }

  handlePagination = (pagination) => {
    const { getRecords } = this.props // params
    getRecords(
      //{
      //params: {
